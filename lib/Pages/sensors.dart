...
void initState() {
    ...
    getDocID().then((value) {
      if (docIDs.isNotEmpty) {
        documentId = docIDs[0];
        _databaseReference = FirebaseDatabase.instance.reference().child('/users/$documentId/Both/Smart Chair/distance');
 ...
 User? user = FirebaseAuth.instance.currentUser;
  ...
  void initState() {
    ...
    getDocID().then((value) {
      if (docIDs.isNotEmpty) {
        documentId = docIDs[0];
        _databaseReference = FirebaseDatabase.instance.reference().child('/users/$documentId/Both/Smart Posture/angle');
        _databaseReference.onValue.listen((event) {
          setState(() {
            gaugeDocIDs = docIDs;
            _sensorData = double.parse(event.snapshot.value.toString());
